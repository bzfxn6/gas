...
for lambda in "$JSON_DIR"/*.json; do
    if [ -f "$lambda" ]; then
        ...
        if [ -f "$source_file" ]; then
            # Calculate SHA256 hash (use shasum for macOS)
            hash=$(shasum -a 256 "$source_file" | cut -d' ' -f1)
            echo "  \"$lambda_name\": \"sha256:$hash\"," >> "$HASH_FILE"
            echo "  Generated hash: sha256:$hash"
        else
...
if [ -f "$HASH_FILE" ]; then
    ...
    if command -v jq &> /dev/null; then
        ...
        for lambda in "$JSON_DIR"/*.json; do
            if [ -f "$lambda" ]; then
                lambda_name=$(basename "$lambda" .json)
                hash=$(grep "\"$lambda_name\":" "$HASH_FILE" | sed 's/.*"sha256:\([^"]*\)".*/\1/')
                echo "  $lambda_name: sha256:$hash"
            fi
        done
    fi
else
