...
resource "aws_sqs_queue" "deadletter_queue" {
  ...
  name                              = each.value.fifo_queue ? "${var.resource_prefix}-${each.key}-dlq.fifo" : "${var.resource_prefix}-${each.key}-dlq"
  fifo_queue                        = each.value.fifo_queue
  kms_master_key_id                 = each.value.kms_master_key_id
  kms_data_key_reuse_period_seconds = each.value.kms_data_key_reuse_period_seconds
  redrive_allow_policy = jsonencode({
    redrivePermission = "byQueue",
    sourceQueueArns   = ["arn:aws:sqs:${var.region}:${var.aws_number_account}:${var.resource_prefix}-${each.key}"]
  })
...
resource "aws_sqs_queue" "sqs_queue" {
  ...
  name                              = each.value.fifo_queue ? "${var.resource_prefix}-${each.key}.fifo" : "${var.resource_prefix}-${each.key}"
